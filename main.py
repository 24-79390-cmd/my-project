...
# Text area where movies are displayed
display = tk.Text(root, height=20, width=75, bg="#1E1E1E", fg="white", wrap=tk.WORD)
display.pack(pady=10)

# Text styles inside the display box
display.tag_configure("title", font=("Arial", 14, "bold"), foreground="#FFD700")
display.tag_configure("desc", font=("Arial", 11))
...
select_btn = tk.Button(btn_frame, text="Select Genre", command=show_movies, bg="#FFD700", font=button_font)
surprise_btn = tk.Button(btn_frame, text="Surprise Me", command=lambda: show_movies(True), bg="#FFD700", font=button_font)
reset_btn = tk.Button(btn_frame, text="Reset", command=reset_ui, bg="#800000", fg="white", font=button_font)
exit_btn = tk.Button(btn_frame, text="Exit", command=root.quit, bg="#800000", fg="white", font=button_font)

# Place buttons in a row
for i, b in enumerate([select_btn, surprise_btn, reset_btn, exit_btn]):
    b.grid(row=0, column=i, padx=6)

# Button hover effect (changes color on mouse hover)
def on_enter(e):
...
for b in [select_btn, surprise_btn, reset_btn, exit_btn]:
    b.bind("<Enter>", on_enter)
    b.bind("<Leave>", on_leave)

# ================== ADDED FEATURE ONLY ================== #
# Watched Movies Checklist
# This feature lets users mark movies they have already watched.

watched_movies = {}

def open_watched_list():
    # Opens a new window with checkboxes
    watched_window = tk.Toplevel(root)
    watched_window.title("Watched Movies Checklist")
    watched_window.geometry("450x500")
    watched_window.configure(bg="#1E1E1E")

    tk.Label(
        watched_window,
        text="✔ Movies You've Watched",
        font=("Arial", 16, "bold"),
        bg="#1E1E1E",
        fg="#FFD700"
    ).pack(pady=10)

    # Scrollable area for long movie lists
    canvas = tk.Canvas(watched_window, bg="#1E1E1E", highlightthickness=0)
    scrollbar = tk.Scrollbar(watched_window, orient="vertical", command=canvas.yview)
    ...
    # Create checkboxes for every movie
    for genre, movie_list in movies.items():
        tk.Label(
            frame,
            text=f"{genre_emojis[genre]} {genre}",
            font=("Arial", 13, "bold"),
            bg="#1E1E1E",
            fg="#FFD700"
        ...
        for movie in movie_list:
            ...
            if title not in watched_movies:
                watched_movies[title] = tk.BooleanVar()

            tk.Checkbutton(
                frame,
                text=title,
                variable=watched_movies[title],
                bg="#1E1E1E",
                fg="white",
                selectcolor="#2E2E2E",
...
# Button to open watched list
watched_btn = tk.Button(
    btn_frame,
    text="Watched List",
    command=open_watched_list,
    bg="#4CAF50",
    fg="white",
    font=button_font
)
watched_btn.grid(row=1, column=0, columnspan=4, pady=8)

watched_btn.bind("<Enter>", on_enter)
watched_btn.bind("<Leave>", on_leave)
